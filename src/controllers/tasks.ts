...
export class TasksController {
	public static getTasks: RequestHandler<RoomIdParam, TasksResponse, RequestWithUser> = async (
		req,
		res,
		next
	) => {
	...
	public static createTask: RequestHandler<RoomIdParam, TaskResponse, TaskRequest> = async (
		req,
		res,
		next
	) => {
		try {
			...
			const { roomId } = req.params;
			const newTask = await TasksService.createTask(+roomId, user.userId, content, status, groupId);
	...
	public static deleteTask: RequestHandler<TaskParams, DeleteTaskResponse, RequestWithUser> =
		async (req, res, next) => {
			try {
				const { user } = req.body;
	...
	public static editTask: RequestHandler<TaskParams, TaskResponse, TaskRequest> = async (
		req,
		res,
		next
	) => {
		try {
			...
			ActivitiesServices.newActivity(+roomId, user.userId, ActivitySphere.TASK, ActivityType.EDIT);

			if (+task!.groupId !== oldTaskState!.groupId) {
				ProgressServices.changeProgress(user.userId, task!.groupId, oldTaskState!.groupId);
			} else if (
