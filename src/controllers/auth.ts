...
import { TokensResponse, RegistrationRequest, LoginRequest } from './auth.types';

export class AuthController {
	public static registration: RequestHandler<undefined, StandardResponse, RegistrationRequest> =
		async (req, res, next) => {
			try {
	...
	public static authentication: RequestHandler<undefined, TokensResponse> = async (
		req,
		res,
		next
	) => {
	...
	public static login: RequestHandler<undefined, TokensResponse, LoginRequest> = async (
		req,
		res,
		next
	) => {
	...
	public static logout: RequestHandler<undefined, StandardResponse> = (_, res) => {
		res.clearCookie(COOKIE_NAME);
		res.json({ resultCode: 0 });
	};

	public static refresh: RequestHandler<undefined, TokensResponse> = (req, res, next) => {
		try {
